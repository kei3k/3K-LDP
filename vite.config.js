...
export default defineConfig(({ mode }) => {
  ...
  plugins: [
    ...
    // Clone Transcript pipeline — STT (Vertex Gemini audio), then duration-matched
    // audio assembly. Client does translation + TTS; server does ffmpeg audio work.
    //   POST /api/clone-tx/stt?id&lang        raw video body → { segments, duration }
    //   POST /api/clone-tx/upload?id&kind&idx raw audio/video body → 'ok'
    //   POST /api/clone-tx/assemble?id        JSON timing → mp4 blob
    {
      name: 'clone-transcript',
      configureServer(server) {
        // ── helpers ──────────────────────────────────────────────────────────
        const ctVideo = (id) => path.join(tmpdir(), `ct_${id}_video.mp4`)
        const ctChunk = (id, idx) => path.join(tmpdir(), `ct_${id}_chunk_${idx}.audio`)
        ...
        // Decompose a playback-speed ratio into atempo factors each in [0.5, 2.0].
        // ratio > 1 = play faster (shorter output); product of factors === ratio.
        ...
        server.middlewares.use('/api/clone-tx', async (req, res) => {
          ...
          // ── STT: extract audio + transcribe via Vertex Gemini ───────────────
          if (pathname === '/stt') {
            ...
            try {
              const a = getAuth()
              if (!a) { res.statusCode = 500; res.end('Vertex chưa cấu hình (.env)'); return }
...
Giữ nguyên ngôn ngữ gốc, KHÔNG dịch. CHỈ trả JSON array, không giải thích.`
          ...
          // ── Assemble: atempo-fit each chunk to its segment slot, mux ────────
          if (pathname === '/assemble') {
            ...
            try {
              ...
              const segments = body.segments || []  // [{idx, start, end}]
