/**
 * Template 2: Landing Page Bán 1 Sản Phẩm (Style HUD Hiển Thị Tốc Độ)
 * 
 * Uses the REAL HTML from buydash.click/hudhienthitocdo.
 * Same approach as template1.js: targeted image replacement + hydration.
 */
import rawHtml from './template2_raw.html?raw';

export function generateTemplate2(productInfo, colors, config = {}) {
  const {
    name = 'Sản phẩm mới',
    description = '',
    benefits = '',
  ...
  if (images && images.length > 0) {
    ...
    console.log(`[Template2] Gallery indices: ${[...galleryUrls.keys()].join(',')}, images: ${images.length}`);
    ...
    console.log(`[Template2] Total gallery replacements: ${replacementCount}`);
  } else {
    console.warn('[Template2] ⚠️ NO IMAGES!');
  }
  ...
  // ============================================
  // 3. CUSTOM FORM FIELDS + TRACKING + HYDRATION
  // ============================================
  const customFieldsHtml = buildCustomFieldsHtml(config.customFormFields || []);
  ...
  // CSS FIX: Only word-wrap (DO NOT change position/height!)
...
// ---- Shared helpers (same as template1.js) ----
...
function buildCustomFieldsHtml(fields) {
  ...
  (fields || []).forEach(field => {
    if (field.type === 'radio' && field.options) {
      ...
      opts.forEach((opt,i) => { html += `<label style="display:block;padding:8px 12px;margin:4px 0;background:#fff;border:1px solid #ddd;border-radius:6px;cursor:pointer;font-size:13px;"><input type="radio" name="${field.name||'combo'}" value="${opt.trim()}" ${i===0?'checked':''} style="margin-right:8px;"> ${opt.trim()}</label>`; });
      html += '</div>';
    } else if (field.type === 'select' && field.options) {
      const opts = field.options.split('\n').filter(o => o.trim());
      html += `<div style="margin-bottom:8px;"><select name="${field.name||'select'}" style="width:100%;padding:10px;border:1px solid #000;border-radius:6px;font-size:13px;background:#fff;"><option value="">${field.label||'Chọn...'}</option>${opts.map(o=>`<option value="${o.trim()}">${o.trim()}</option>`).join('')}</select></div>`;
    } else if (field.type === 'textarea') {
      html += `<div style="margin-bottom:8px;"><textarea name="${field.name||'note'}" placeholder="${field.label||'Ghi chú'}" style="width:100%;padding:10px;border:1px solid #000;border-radius:6px;font-size:13px;min-height:60px;box-sizing:border-box;"></textarea></div>`;
    } else {
      html += `<div style="margin-bottom:8px;"><input type="${field.type||'text'}" name="${field.name||'field'}" placeholder="${field.label||''}" style="width:100%;padding:10px;border:1px solid #000;border-radius:6px;font-size:13px;box-sizing:border-box;background:#fff;"></div>`;
    }
  });
  return html;
}
    ...
    if (cfHtml) {
      document.querySelectorAll('form').forEach(function(form) {
        ...
        form.querySelectorAll('[class*="button-css"]').forEach(function(b) {
          if ((b.innerText||'').trim().length > 1) submitBtn = b.closest('[id^="w-"]');
        });
